...
def get_brain_response(text, history):
    """Lazy-load the brain response from main.py to avoid circular imports."""
    try:
        import main
        # agent_respond(prompt, history, user_name) returns (reply, new_history)
        reply, _ = main.agent_respond(text, history=history)
        return reply, None
...
OFFSET_FILE = os.path.join(PROJECT_ROOT, "core_os/memory/telegram_offset.txt")

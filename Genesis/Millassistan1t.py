...
# --- CONFIGURATION ---
CONFIG = {
    'model_name': 'gpt-oss:120b-cloud',  # Verify you have this model pulled in Ollama
    'host': '0.0.0.0',
    'port': 8000,
...
def extract_json(text: str) -> Optional[Dict]:
    """Robust JSON extraction using Regex to prevent crashing on bad output."""
    try:
        # Looks for the first valid JSON object block { ... }
...
class MediaController:
    ...
    def play_youtube(self, query: str) -> str:
        try:
            ...
            # Spawn MPV detached so it doesn't block the server
            subprocess.Popen([
                'mpv', 
...
class SystemTools:
    ...
    def inspect_file(self, path: str) -> str:
        """Reads a file to put into context (RAG-Lite)."""
        try:
    ...
    def check_capabilities(self) -> str:
        ...
        status = "Online"
        modules = ["MediaController", "SystemTools", "EmailManager", "OllamaAsync"]
...
class MillaAgent:
    def __init__(self):
        ...
        self.ollama_client = AsyncClient() # Async client prevents freezing
        self._init_db()
...
PROTOCOL:
1. If you need information or need to do something, output the JSON.
...
"""
    ...
    async def process_request(self, user_input: str) -> str:
        ...
        while current_turn < max_turns:
            ...
            # ASYNC Call - This prevents the freeze
            response = await self.ollama_client.chat(model=CONFIG['model_name'], messages=messages)
            ...
            if tool_data and 'tool' in tool_data:
                ...
                # Execute Tool (Sync tools wrapped in async flow)
                if tool_name in self.tools:
                    try:
                        # We run the tool execution; for heavy tools we could await them
